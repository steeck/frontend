    ...
    <v-container pa-0 mb-5 wrap class="area-content-card" :class="{'wide' : viewWide, 'relative' : !viewWide}">
      <v-layout row wrap class="c-all">
        <v-flex xs12 class="cc-top">
          <!--상단 유저 관련 내용 및 메뉴 -->
          <v-layout row wrap align-center px-3 py-2>
            <v-avatar color="grey lighten-4" size="40">
              <v-img :src="'https://steemitimages.com/u/' + item.author + '/avatar'" alt="avatar"></v-img>
            </v-avatar>
            <div class="ml-3">
              <div class="card-author">
                <router-link :tag="'a'" :to="{name: 'User', params: {username: item.author}}"
                             target="_blank" class="link">{{ item.author }}</router-link>
              </div>
        ...
        <v-flex pa-0 xs12 class="cc-post"
          ...
          <v-carousel hide-delimiters hide-controls :cycle="false" height="fit-content"
                      class="elevation-0"
                      v-model="cardIndex" ref="carousel">
            <v-carousel-item v-for="(card, i) in content.data.contents" :key="(i+1)"
            >
              <v-flex xs12 pa-0 v-if="card.url !== null">
                <v-img :src="card.url ? card.url : ''" max-height="500"></v-img>
              </v-flex>
            </v-carousel-item>
          </v-carousel>

          <v-tabs v-model="cardIndex" show-arrows centered hide-slider
                  dark
                  color="white"
                  class="ccp-post">
            <v-tab
            ...
            >
              <!--<v-flex xs12 v-if="card.url !== null" mt-2 class="ccpc-img">-->
                <!--<v-img contain :src="card.url ? card.url : ''"  max-height="500"></v-img>-->
              <!--</v-flex>-->
            ...
            <div>
              <v-layout row wrap pa-1 xs12>
                ...
                <div class="block-cus_icon" @click="openVote" :style="'position:relative;'">
                  <v-icon size="20" color="primary" v-text="isVoted ? 'lens' : 'panorama_fish_eye'"
                  ></v-icon>
                  <v-icon size="20" :color="isVoted ? 'rgb(255,255,255)' : 'primary'"
                          :style="'position:absolute; left:0; top:0;'" v-text="'keyboard_arrow_up'"></v-icon>
                </div>
